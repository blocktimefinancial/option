...
=======
    ...
    return await client.server.prepareTransaction(transaction, client.network)
>>>>>>> 1f2a01a8bdf6fcae018186ecb1d7f008169e28ce
}
...
function getSorobanResultValue(result) {
  ...
  if (value.value() === false)
    //if footprint's data is different from the contract execution data, the result is false
    return undefined;
  return value;
}
  ...
  switch (AssetType[assetType]) {
    ...
    case AssetType.Stellar:
      return {
        ...
        code: Address.contract(xdrAsset[1].value().value()).toString(),
      };
    default:
...
class OracleClient {
    ...
    const configScVal = xdr.ScVal.scvMap([
      ...
      new xdr.ScMapEntry({
        ...
        val: xdr.ScVal.scvVec(
          config.assets.map((asset) => buildAssetScVal(asset))
        ),
      }),
      new xdr.ScMapEntry({
  ...
  async bump(source, ledgersToLive, options = { fee: 100 }) {
    return await buildTransaction(
      this,
      source,
      this.contract.call("bump", xdr.ScVal.scvU32(ledgersToLive)),
      options,
      this.network
    );
  }
  ...
  async setPrice(source, updates, timestamp, options = { fee: 100 }) {
    const scValPrices = xdr.ScVal.scvVec(
      updates.map((u) => nativeToScVal(u, { type: "i128" }))
    );
    return await buildTransaction(
      this,
      source,
  ...
  async lastTimestamp(source, options = { fee: 100 }) {
    return await buildTransaction(
      this,
      source,
      this.contract.call("last_timestamp"),
      options,
      this.network
    );
  }
  ...
  async submitTransaction(transaction, signatures = []) {
    ...
    if (submitResult.status !== "PENDING") {
      const error = new Error(
        `Transaction submit failed: ${submitResult.status}`
      );
    ...
    if (response.status === "FAILED") {
      const error = new Error(
        `Transaction submit failed, result: ${response.status}`
      );
  ...
  static parseAdminResult(result) {
    const adminBuffer = getSorobanResultValue(result)
      ?.value()
      ?.value()
      ?.value();
    if (adminBuffer === undefined) return null;
    const adminPublicKey = new Keypair({
      type: "ed25519",
      publicKey: adminBuffer,
    });
